...
```cypher
MATCH (org:GH_Organization {actions_sha_pinning_required: false}) RETURN org
```
...
```cypher
MATCH p=(:GH_Repository)-[:GH_HasSecretScanningAlert]->(alert:GH_SecretScanningAlert {state: 'open', validity: 'active'}) RETURN p
```
...
```cypher
MATCH (org:GH_Organization {advanced_security_enabled_for_new_repositories: false}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {actions_allowed_actions: 'all'}) RETURN org
```
...
```cypher
MATCH (app:GH_AppInstallation {repository_selection: 'all'}) RETURN app
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {enforce_admins: false})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {allows_deletions: true})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {allows_force_pushes: true})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {require_code_owner_reviews: false})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {required_pull_request_reviews: false})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {requires_status_checks: false})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {require_last_push_approval: false})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_BranchProtectionRule {dismisses_stale_reviews: false})-[:GH_ProtectedBy]->(:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(actor)-[:GH_BypassPullRequestAllowances]->(rule:GH_BranchProtectionRule)-[:GH_ProtectedBy]->(branch:GH_Branch) RETURN p
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasRole|GH_HasBaseRole|GH_MemberOf*1..]->(:GH_RepoRole)-[:GH_PushProtectedBranch|GH_BypassBranchProtection]-(r:GH_Repository) MATCH p1=(:GH_User)-[:GH_BypassPullRequestAllowances|GH_RestrictionsCanPush]->(rule:GH_BranchProtectionRule)-[:GH_ProtectedBy]->(b:GH_Branch) RETURN p,p1
```
...
```cypher
MATCH (o:GH_Organization) WHERE o.default_repository_permission <> 'none' RETURN o
```
...
```cypher
MATCH (org:GH_Organization {dependabot_alerts_enabled_for_new_repositories: false}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {dependabot_security_updates_enabled_for_new_repositories: false}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {dependency_graph_enabled_for_new_repositories: false}) RETURN org
```
...
```cypher
MATCH p=(:GH_Repository)-[:GH_HasEnvironment]->(env:GH_Environment {can_admins_bypass: true}) RETURN p
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasPersonalAccessToken]->(token:GH_PersonalAccessToken {token_expired: true}) RETURN p
```
...
```cypher
MATCH (ei:GH_ExternalIdentity) WHERE ei.scim_identity_username = '' RETURN ei
```
...
```cypher
MATCH p=(src)-[:CanAssumeIdentity]->(cred:AZFederatedIdentityCredential) RETURN p
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasBaseRole|GH_HasRole|GH_MemberOf*1..3]->(role:GH_OrgRole) WHERE role.short_name CONTAINS 'all_repo_' RETURN p
```
...
```cypher
MATCH p=(s)-[]->(d:GH_User) WHERE s:AZUser OR s:OktaUser RETURN p
```
...
```cypher
MATCH (org:GH_Organization {members_can_change_repo_visibility: true}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {members_can_create_pages: true}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {members_can_create_public_repositories: true}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {members_can_delete_repositories: true}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {members_can_fork_private_repositories: true}) RETURN org
```
...
```cypher
MATCH (org:GH_Organization {members_can_invite_outside_collaborators: true}) RETURN org
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasRole]->(:GH_OrgRole {short_name:'owners'}) RETURN p
```
...
```cypher
MATCH (o:GH_Organization) WHERE o.two_factor_requirement_enabled = false RETURN o
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasPersonalAccessToken]->(token:GH_PersonalAccessToken {repository_selection: 'all'}) RETURN p
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasPersonalAccessTokenRequest]->(req:GH_PersonalAccessTokenRequest) RETURN p
```
...
```cypher
MATCH (repo:GH_Repository {visibility: 'private', allow_forking: true}) RETURN repo
```
...
```cypher
MATCH p=(role:GH_OrgRole {type:'custom'})-[r]->(dest) WHERE dest:GH_Organization OR dest:GH_OrgRole RETURN p
```
...
```cypher
MATCH p=()-[:SyncedToGHUser]->(:GH_User)-[:GH_HasRole]->(:GH_OrgRole {short_name:'owners'}) RETURN p
```
...
```cypher
MATCH (repo:GH_Repository {private: false}) RETURN repo
```
...
```cypher
MATCH (org:GH_Organization {secret_scanning_push_protection_enabled_for_new_repositories: false}) RETURN org
```
...
```cypher
MATCH p=(actor)-[:GH_RestrictionsCanPush]->(rule:GH_BranchProtectionRule)-[:GH_ProtectedBy]->(branch:GH_Branch) RETURN p
```
...
```cypher
MATCH (repo:GH_Repository {secret_scanning: 'disabled'}) RETURN repo
```
...
```cypher
MATCH p=(:GH_Repository)-[:GH_HasWorkflow]->(:GH_Workflow) RETURN p
```
...
```cypher
MATCH p=(OIP:GH_SamlIdentityProvider)-[:GH_HasExternalIdentity]->(EI:GH_ExternalIdentity) MATCH p1=(OIP)<-[:GH_HasSamlIdentityProvider]-(:GH_Organization) MATCH p2=(EI)-[:GH_MapsToUser]->() RETURN p,p1,p2
```
...
```cypher
MATCH p=(repo:GH_Repository)-[:GH_HasSecretScanningAlert]->(:GH_SecretScanningAlert {state:'open'}) RETURN p
```
...
```cypher
MATCH (org:GH_Organization {secret_scanning_enabled_for_new_repositories: false}) RETURN org
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasRole|GH_HasBaseRole|GH_MemberOf*1..]->(:GH_RepoRole)-[:GH_WriteRepoContents]->(:GH_Repository)-[:GH_HasSecret]->(s) WHERE s:GH_RepoSecret OR s:GH_OrgSecret RETURN p
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasRole]->(:GH_TeamRole)-[:GH_AddMember]->(team:GH_Team) MATCH p1=(team)<-[:GH_MemberOf]-(:GH_Team)<-[:GH_AddMember]-(:GH_TeamRole)<-[:GH_HasRole]-(:GH_User) RETURN p,p1
```
...
```cypher
MATCH p=(:GH_User)-[:GH_HasRole]->(:GH_TeamRole)-[:GH_MemberOf*1..]->(:GH_Team) RETURN p
```
...
```cypher
MATCH p=(repo:GH_Repository)-[:GH_HasBranch]-(:GH_Branch {protected: false}) RETURN p
```
...
```cypher
MATCH p=(repo:GH_Repository)-[:GH_HasWorkflow]->(:GH_Workflow) MATCH p1=(repo)-[:GH_HasBranch]->(branch:GH_Branch) WHERE repo.default_branch = branch.short_name RETURN p1
```
...
```cypher
MATCH p=(repo:GH_Repository)-[:GH_HasBranch]-(branch:GH_Branch {protected: false}) WHERE repo.default_branch = branch.short_name RETURN p
```
...
```cypher
MATCH (org:GH_Organization {web_commit_signoff_required: false}) RETURN org
```
